...
from timm.utils.metrics import F1_Score, AUC, Count,PR_AUC, opt_f1
...
class WSI(Dataset):
    def __init__(self):
        self.im= glob('/hpc/dla_patho/hassan_/LVI/val/positive' + '/*.jpg') + glob('/hpc/dla_patho/hassan_/LVI/val/negative' + '/*.jpg')
    ...
    def __getitem__(self, idx):
        im_patch = Image.open(self.im[idx])
        im_patch = self.transform(im_patch)
        target = 0 if 'negative' in self.im[idx] else 1
        return im_patch, target
...
def validate(args):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            """additional metrics"""
            count_pred_tmp = Count(output.detach().cpu(), target.detach().cpu())
            count_pred[0] += count_pred_tmp[0]
            count_pred[1] += count_pred_tmp[1]
            count[0] += len(target) - target.sum()
            count[1] += target.sum()
    ...
    f1_score= F1_Score(count_pred, count)
    auc= AUC(outputs, targets)
    pr_auc= PR_AUC(outputs, targets)
    opt_f1(thresholds, np.array(outputs), np.array(targets))

    print('\nnegative:{0}/{1}; {2:.2f}, positive:{3}/{4}; {5:.2f}'.format(count_pred[0], count[0],
    100*count_pred[0]/count[0], count_pred[1], count[1], 100*count_pred[1]/count[1]))
    ...
    print("\n\n")
    _logger.info('loss {:.3f}, acc@1 {:.3f}, f1 {:.3f}, auc {:.3f}, pr_auc {:.3f}'.format(
       results['loss'], results['top1'], results['f1'], results['auc'], results['pr_auc']))

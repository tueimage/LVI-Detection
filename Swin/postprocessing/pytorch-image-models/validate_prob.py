...
parent_dir = '/hpc/dla_patho/hassan_/LVI/probs_' + args.split + '/'
...
class WSI(Dataset):
    def __init__(self, name):
        self.name= name
        self.im= OpenSlide(data_dir + name +'/'+ name + '.ndpi')
        self.file= h5py.File(coord_dir + name + ".h5", "r")
        self.coords= np.array(self.file['coords'])
        self.d_0 = self.d_1 = 2048
        self.m= 2
        
        # Define the transformation pipeline
        transform = transforms.Compose([
                    transforms.Resize(248, interpolation=transforms.InterpolationMode.BICUBIC),  # Resize to 248 using bicubic interpolation
                    transforms.CenterCrop((224, 224)),  # Center crop to 224x224
                    timm.data.transforms.ToNumpy(),  # Convert to numpy array (from timm)
                ])
    ...
    def __getitem__(self, idx):
        r_0, r_1 = self.coords[idx]
        im_patch= self.im.read_region((r_0, r_1), self.m, (self.d_0//(2**self.m), self.d_1//(2**self.m)))
        im_patch= im_patch.convert('RGB')
        im_patch = self.transform(im_patch)
        return im_patch, 0
...
def validate(args):
    ...
    "List of slide names"
    if args.split == "test":
        slides= ['slide_53','slide_63','slide_88','slide_41','slide_36','slide_7','slide_80','slide_67','slide_30','slide_57','slide_55','slide_65','slide_24','slide_15','slide_54','slide_64','slide_37','slide_81','slide_56']
    else:
        slides = ['slide_89','slide_28','slide_84','slide_14','slide_39','slide_6','slide_26','slide_59','slide_70','slide_82','slide_0','slide_10','slide_74','slide_85','slide_27','slide_60','slide_1','slide_75']
    
    "Compute the probs and save them"   
    for img_name in slides:    
        ...
        with torch.no_grad():
            ...
            outputs = torch.concatenate(outputs, axis= 0)

            # save the output
            np.save(parent_dir + '/' + img_name + ".npy", outputs.detach().cpu().numpy())

            # print the name of the slide
            print(img_name)
    return outputs

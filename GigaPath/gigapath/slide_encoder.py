...
def create_model(pretrained: str, model_arch: str, in_chans: int, local_dir: str = os.path.join(os.path.expanduser("~"), ".cache/"), **kwargs):
    model = gigapath_slide_enc12l768d(pretrained=False, in_chans=in_chans, **kwargs)
    ...
    if os.path.exists(local_path):
        ...
        if len(unexpected_keys) > 0:
            for k in unexpected_keys:
                print("Unexpected ", k)
        print("\n\033[92m Successfully Loaded Pretrained GigaPath model from {} \033[00m".format(pretrained)) 
    else:

...
from utils import (get_optimizer, get_loss_function, \
                  get_records_array,
                  log_writer, adjust_learning_rate, print_network,
                  EarlyStopping)
...
def train(dataloader, fold, args):
    ...
    for i in range(args.epochs):
        ...
        if val_loader is not None:
            val_records, stop = evaluate(val_loader, model, fp16_scaler, loss_fn, i, args, early_stopping= early_stopping)
    ...
    # test the model
    val_records, _ = evaluate(val_loader, model, fp16_scaler, loss_fn, i, args, test=True)
    test_records, _ = evaluate(test_loader, model, fp16_scaler, loss_fn, i, args, test=True)
...
def train_one_epoch(train_loader, model, fp16_scaler, optimizer, loss_fn, epoch, args):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16 if args.fp16 else torch.float32):
            ...
            pos_weight = len(label) / (label.sum())
            neg_weight = len(label) / (len(label) - label.sum())

            loss = torch.mean(loss * (pos_weight * label + neg_weight * (1 - label)))
            loss /= args.gc
    ...
    info = '\nEpoch: {}, Loss_Train: {:.4f}, Micro FROC_Train: {:.4f},  Micro AUPRC_Train: {:.4f}'.format(epoch, records['loss'], records['micro_froc'], records['micro_auprc'])
    print(info)
    return records


def evaluate(loader, model, fp16_scaler, loss_fn, epoch, args, test=False, early_stopping = None):
    model.eval()
    ...
    if test:
        ...
        if task_setting == 'multi_label':
            info = 'Epoch: {}, Loss_Val: {:.4f}, Micro FROC_Val: {:.4f}, Micro AUPRC_Val: {:.4f}'.format(epoch, records['loss'], records['micro_froc'], records['micro_auprc'])
        print(info)
        print('\n')

    if early_stopping:
        early_stopping(epoch, records['micro_froc'], model, ckpt_name=os.path.join(args.save_dir, 'fold_' + str(0), "checkpoint.pt"))

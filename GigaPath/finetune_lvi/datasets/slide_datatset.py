...
class SlideDatasetForTasks(Dataset):
    def __init__(self,
        ...
        # set up the task
        self.setup_data(data_df, splits, task_config.get('setting', 'multi_label'))
    ...
    def setup_data(self, df: pd.DataFrame, splits: list, task: str='multi_label'):
        ...
        self.slide_data, self.images = prepare_data_func(df, splits)
...
class SlideDataset(SlideDatasetForTasks):
    ...
    def shuffle_data(self, images: torch.Tensor, coords: torch.Tensor, labels: torch.Tensor) -> tuple:
        '''Shuffle the serialized images and coordinates'''
        indices = torch.randperm(len(images))
        images_ = images[indices]
        coords_ = coords[indices]
        labels_ = labels[indices]
        return images_, coords_, labels_
    ...
    def get_images_from_path(self, img_path: str, label_path: str) -> dict:
        ...
        if '.pt' in img_path:
            ...
            # if shuffle the data
            if self.shuffle_tiles and self.train:
                images, coords, labels = self.shuffle_data(images, coords, labels)

            if images.size(0) > self.max_tiles:
                images = images[:self.max_tiles, :]
            if coords.size(0) > self.max_tiles:
                coords = coords[:self.max_tiles, :]
            if labels.size(0) > self.max_tiles:
                labels = labels[:self.max_tiles, :]
    ...
    def get_one_sample(self, idx: int) -> dict:
        ...
        if 'pt_files' in self.root_path.split('/')[-1]:
            ...
            label_path = os.path.join(self.root_path, slide_id.replace(".svs", "") + '.npy')
        else:
            slide_path = os.path.join(self.root_path, slide_id.replace(".svs", "") + '.h5')
            label_path = os.path.join(self.root_path, slide_id.replace(".svs", "") + '.npy')

        # get the slide images
        data_dict = self.get_images_from_path(slide_path, label_path)
